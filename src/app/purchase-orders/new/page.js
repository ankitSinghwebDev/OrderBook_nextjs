...
const starterSuppliers = [
  { id: 'sup-1', name: 'Acme Supplies', email: 'orders@acme.com', phone: '555-1111' },
  { id: 'sup-2', name: 'Global Traders', email: 'sales@global.com', phone: '555-2222' },
];

const starterAddresses = [
  { id: 'addr-1', label: 'HQ Warehouse', line1: '123 Industrial Ave', city: 'NYC', country: 'USA' },
  { id: 'addr-2', label: 'Secondary Hub', line1: '55 Logistics Park', city: 'Dallas', country: 'USA' },
];
...
export default function NewPurchaseOrderPage() {
  ...
  const [selectedSupplier, setSelectedSupplier] = useState(starterSuppliers[0]?.id || null);
  const [selectedAddress, setSelectedAddress] = useState(starterAddresses[0]?.id || null);
  ...
  const totals = useMemo(() => {
    ...
    const taxTotal = items.reduce(
      (sum, i) =>
        sum +
        ((Number(i.qty) || 0) * (Number(i.rate) || 0) * (Number(i.tax) || 0)) / 100,
      0
    );
  ...
  const addItem = () =>
    setItems((prev) => [...prev, { name: '', description: '', qty: 1, rate: 0, tax: 0 }]);
    ...
    <div className="min-h-screen py-10" style={{ background: 'var(--background)' }}>
      <div className="mx-auto max-w-6xl px-4 space-y-6">
        <div>
          ...
          <Text type="secondary">Zoho-style layout with supplier, delivery, and bulk items.</Text>
        </div>
        ...
        >
          <div className="grid gap-4 lg:grid-cols-3">
            <Card
              ...
              extra={
                <Button type="link" icon={<PlusOutlined />} onClick={() => setAddSupplierOpen(true)}>
                  New supplier
                </Button>
              }
            >
            ...
            >
              ...
              <Form.Item label="Delivery instructions" name="deliveryAddress">
                <Input.TextArea rows={3} placeholder="Dock, contact, timing..." />
              </Form.Item>
            </Card>
            ...
            <Card
              ...
              style={{ background: 'var(--card)', borderColor: 'var(--border)' }}
            >
              <Form.Item label="PO Number" name="orderNumber" rules={[{ required: true }]}>
          ...
          <Card
            ...
            extra={
              <Space>
                <Button icon={<InboxOutlined />} onClick={() => setBulkModalOpen(true)}>
                  Bulk add
                </Button>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            >
              ...
              <Form.Item label="Terms & Conditions" name="notes">
                <Input.TextArea rows={3} placeholder="Payment terms, delivery window…" />
              </Form.Item>
            </Card>
            ...
            >
              <Form.Item label="Internal notes" name="internalNotes">
                <Input.TextArea rows={5} placeholder="Visible to your team only" />
              </Form.Item>
